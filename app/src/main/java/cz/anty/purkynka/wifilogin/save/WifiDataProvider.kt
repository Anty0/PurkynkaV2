...
import cz.anty.purkynka.utils.PrefNames.FILE_NAME_WIFI_DATA
...
class WifiDataProvider : VersionedContentProviderPreferences<SharedPreferences>(AUTHORITY, WifiData.SAVE_VERSION) {
    ...
    override fun onPreparePreferencesProvider(): ISharedPreferencesProvider<SharedPreferences> {
        return BasicSharedPreferencesProvider(context, FILE_NAME_WIFI_DATA, Context.MODE_PRIVATE)
    }

    override fun onUpgrade(editor: SharedPreferences.Editor, from: Int, to: Int) {
        WifiData.onUpgrade(editor, from, to)
    }
}